...
- `了解本地模式`: explains that real mode runs on the user's own computer and requires active authorization.
...
The activation CTA routes to the user-facing local-mode explanation. It does not automatically import activity or read local files.

User-facing copy should say:

```text
真实本地模式需要你在本机运行，并主动授权。授权后，OpenButler 会开始整理今日概览、时间线、管家提醒和依据。
```
...
- `了解本地模式` navigates to the local-mode explanation.
...
- `了解本地模式` routes to the local-mode explanation,
