...
class DesktopStatusTests(unittest.TestCase):
    ...
    def test_desktop_status_is_loopback_strict_and_redacted(self) -> None:
        ...
        self.assertTrue(payload["first_use"]["activation_required"])
        self.assertEqual(payload["first_use"]["local_mode_stage"], "ready_for_preview")
        self.assertEqual(payload["first_use"]["today_preview"]["kind"], "dry_run_only")
        self.assertTrue(payload["first_use"]["today_preview"]["available"])
        self.assertFalse(payload["first_use"]["today_preview"]["database_written"])
        self.assertFalse(payload["first_use"]["today_preview"]["real_activity_titles_returned"])
        self.assertFalse(payload["first_use"]["today_preview"]["screenshot_paths_returned"])
        self.assertFalse(payload["first_use"]["today_preview"]["raw_output_returned"])
