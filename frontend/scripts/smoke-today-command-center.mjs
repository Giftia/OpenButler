...
try {
  ...
  const profileDir = join(tmpdir(), `openbutler-today-command-${Date.now()}`);
  ...
  const result = await evaluate(cdp, `(() => {
    ...
    const visibleText = Array.from(document.querySelectorAll('body *'))
      .filter((node) => {
        ...
        return style.display !== 'none' && style.visibility !== 'hidden' && rect.width > 0 && rect.height > 0 && rect.top < innerHeight;
      })
    ...
    return {
      ...
      firstScreenText: visibleText.slice(0, 1000)
    };
  })()`);
