...
export function privacyModeLabel(value: unknown): string {
  return value === "strict" ? "只在本机整理" : "允许授权联网";
}
...
export function userFacingDemoText(value: unknown, fallback = "这是一条演示记录，用来展示 OpenButler 如何整理本地线索。"): string {
  ...
  if (normalized.includes("check openbutler inbox")) return "有一条提醒被整理出来，适合现在确认一下。";
  if (normalized.includes("run local verification")) return "连续坐着一段时间后，管家建议短暂活动一下。";
  if (normalized.includes("review productization docs")) return "会议后留下了一项待确认事项，适合稍后收尾。";
  if (normalized.includes("openbutler coding block")) return "样例线索显示钥匙最后出现在玄关托盘附近。";
